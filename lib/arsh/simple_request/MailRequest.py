...
class SimpleMailRequest:
    ...
    def __init__(self, requester, type, object=None, request=None):
        """
            :type request: rezgh.simple_request.models.SimpleRequest
        """
    ...
    def admins_distance(self, admins):
        ...
        for admin in admins:
            ...
            if not admins_location: continue
            admins_distance.append([admin,admins_location.calculate_distance_to(LatLng.parse(str(self._object.glatlng)))])
    ...
    def eligible_responder(self, distances):
        ...
        if eligible_admins : return eligible_admins[0]
            ...
            if isinstance(field, models.Field):
                ...
                if isinstance(field, models.ManyToManyField):
                    if value and value != '[]':
                        ...
                        value = u'، '.join([unicode(field.rel.to.objects.get(pk=vid)) for vid in value[1:-1].split(',')])
                    else:
                        value = u''
            else:
                name = field['name']
    ...
    def create(requester, type, object=None, continue_thread=True):
        ...
        if continue_thread:
            try:
                ...
                req = SimpleRequest.objects.get(requester=requester, request_object_id=object.id, object_content_type=ct
                    , type=type)
